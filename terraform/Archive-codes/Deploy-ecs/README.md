...
### What Do Capacity Providers Do?

Capacity providers bridge the gap between your ECS services (what you want to run) and the underlying compute infrastructure (where it runs). They are responsible for:

1. **Infrastructure Abstraction** — Capacity providers decouple task scheduling from infrastructure management. Instead of manually provisioning and tracking EC2 instances, you declare *what* compute you need and the capacity provider handles *how* it is supplied.

2. **Automatic Scaling of Compute Resources** — When associated with an Auto Scaling Group (EC2 launch type), a capacity provider uses **managed scaling** to automatically add or remove EC2 instances based on the number of tasks ECS needs to place. Without a capacity provider, you would have to create and tune your own CloudWatch alarms and scaling policies.

3. **Cost Optimization via Bin-Packing** — Capacity providers with `target_capacity` set to 100% ensure instances are fully utilized before new ones are launched. You can also lower the target (e.g., 80%) to keep a buffer for faster task placement at the expense of slightly higher cost.

4. **Mixed Compute Strategies** — By assigning multiple capacity providers to a single cluster or service, you can split traffic between different compute tiers. For example:  
   - Run a **base** of 2 tasks on `FARGATE` (reliable, always available)  
   - Burst remaining tasks onto `FARGATE_SPOT` at a 4:1 weight ratio (cheaper, interruptible)  
   - Or mix Fargate with a custom EC2 provider for GPU-backed workloads  

5. **Managed Termination Protection** — For EC2-based providers, enabling managed termination protection prevents the ASG from terminating an instance that still has running tasks, avoiding unexpected task interruptions during scale-in events.

6. **Default Strategy at the Cluster Level** — The `default_capacity_provider_strategy` lets you set a cluster-wide default so that any service created without an explicit strategy automatically inherits the cluster's preferred capacity distribution. This reduces boilerplate and ensures consistency.

**In short:** capacity providers answer the question *"Where should ECS run my tasks, and how should it get the compute to do so?"* — whether that compute is serverless Fargate, discounted Fargate Spot, or self-managed EC2 instances scaled automatically by ECS.
