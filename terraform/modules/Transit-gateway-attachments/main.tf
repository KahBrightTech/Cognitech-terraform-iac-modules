...
resource "aws_ram_resource_share" "main" {
  count = var.tgw_attachments.ram.enabled ? 1 : 0
...
resource "aws_ram_principal_association" "main" {
  count = var.tgw_attachments.ram.enabled && length(var.tgw_attachments.ram.principals) > 0 ? length(var.tgw_attachments.ram.principals) : 0
