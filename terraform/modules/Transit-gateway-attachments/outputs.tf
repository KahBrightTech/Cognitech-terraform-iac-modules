...
output "ram_resource_share_arn" {
  ...
  value       = var.tgw_attachments.ram.enabled ? aws_ram_resource_share.main[0].arn : null
}

output "ram_resource_share_id" {
  description = "The ID of the RAM resource share (if enabled)"
  value       = var.tgw_attachments.ram.enabled ? aws_ram_resource_share.main[0].id : null
}

output "ram_sharing_enabled" {
  description = "Whether RAM sharing is enabled for this TGW attachment"
  value       = var.tgw_attachments.ram.enabled
}
