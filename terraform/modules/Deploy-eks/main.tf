...
resource "aws_eks_pod_identity_association" "pia" {
  for_each        = var.eks.create_service_accounts && var.eks.eks_pia != null && var.eks.service_accounts != null && var.eks.service_accounts.enable_eks_pia ? { for item in var.eks.eks_pia : item.key => item } : {}
  cluster_name    = aws_eks_cluster.eks_cluster.name
