...
locals {
  ...
  lb_name        = var.load_balancer.name != null ? "${var.common.account_name_abr}-${var.load_balancer.vpc_name_abr}-${var.common.region_prefix}-${var.load_balancer.name}-${local.lb_type_suffix}" : "${var.common.account_name_abr}-${var.load_balancer.vpc_name_abr}-${var.common.region_prefix}-${local.lb_type_suffix}"
}
...
resource "aws_lb" "main" {
  ...
  subnets                    = var.load_balancer.type == "network" && var.load_balancer.subnet_mappings != null && length(var.load_balancer.subnet_mappings) > 0 ? null : var.load_balancer.subnets
  enable_deletion_protection = var.load_balancer.enable_deletion_protection
  # If private IPs specified (only for NLB)
  dynamic "subnet_mapping" {
    for_each = var.load_balancer.type == "network" && var.load_balancer.subnet_mappings != null && length(var.load_balancer.subnet_mappings) > 0 ? var.load_balancer.subnet_mappings : []
    content {
...
resource "aws_lb_listener" "default" {
  count             = var.load_balancer.create_default_listener && var.load_balancer.default_listener != null && var.load_balancer.type == "application" ? 1 : 0
  load_balancer_arn = aws_lb.main.arn
  port              = var.load_balancer.default_listener.port
  protocol          = var.load_balancer.default_listener.protocol
  ssl_policy        = var.load_balancer.default_listener.protocol == "HTTPS" ? var.load_balancer.default_listener.ssl_policy : null
  certificate_arn   = var.load_balancer.default_listener.protocol == "HTTPS" ? var.load_balancer.default_listener.certificate_arn : null
