...
resource "aws_subnet" "quaternary" {
  ...
  vpc_id               = var.vpc_id
  availability_zone    = var.private_subnets.quaternary_availabilty_zone
  availability_zone_id = var.private_subnets.quaternary_availabilty_zone_id
  cidr_block           = var.private_subnets.quaternary_cidr_block
  tags = merge(var.common.tags,
    {
      Name = "${var.common.account_name}-${var.common.region_prefix}-${var.private_subnets.name}-quaternary"
    }
  )
}
